...
yearly_monthly_counts_df = filtered_df.groupBy("year", "month").agg(
    count(expr("case when descript like '%STOLEN AUTOMOBILE%' then 1 end")).alias(
        "stolen_count"
    ),
...
yearly_monthly_percentages_df = yearly_monthly_counts_df.withColumn(
    "stolen_pct",
    (yearly_monthly_counts_df["stolen_count"] * 100)
    / yearly_monthly_counts_df["total_count"],
).withColumn(
    "theft_pct",
    (yearly_monthly_counts_df["theft_count"] * 100)
    / yearly_monthly_counts_df["total_count"],
)
...
grouped_df = filtered_df.groupBy("dayOfWeek", "street").agg(count("*").alias("count"))
...
final_df = top_streets_df.groupBy("dayOfWeek").agg(
    sum("count").alias("totalThefts"),
    collect_list(struct("street", "count")).alias("topStreets"),
)

# Project and sort the final result
result_df = final_df.select("dayOfWeek", "topStreets", "totalThefts").orderBy(
    "dayOfWeek"
)
...
grouped_df = filtered_df.groupBy(hour("timestamp").alias("hour")).agg(
    count("*").alias("count")
)
...
(
    yearly_monthly_percentages_df.write.format("mongodb")
    .mode("append")
    .option("connection.uri", mongodb_uri)
    .option("database", "SanFrancisco")
    .option("collection", "sfpd_aggregations")
    .save()
)
(
