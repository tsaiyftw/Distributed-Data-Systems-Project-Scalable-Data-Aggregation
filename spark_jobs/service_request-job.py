...
grouped_df = service_requests_df.groupBy(
    "year", "category").agg(spark_sum(expr("1")).alias("count"))

# Find the most common category for each year
window_spec = Window.partitionBy("year").orderBy(col("count").desc())
ranked_df = grouped_df.withColumn("rank", row_number().over(
    window_spec)).filter(col("rank") == 1).drop("rank")

# Sum the total count of documents for each year
total_count_df = grouped_df.groupBy("year").agg(
...
result_df.select("year", "category", "count", "percentage").orderBy(
    "year").show(truncate=False)
